...
namespace Tests.IntegrationTests
{
    public class LightBulbCommandIntegrationTest : BaseControllerTest
    {
        ...
        [Fact]
        public async Task
            GivenNewLightBulbCommand_WhenLightBulbCommandNumberHasBeenReached_ThenPostAsyncShouldReturnPaymentRequiredStatusCode()
        {
            using HttpClient client = await GetClientAsync(GetType().Name);

            await client.PostAsJsonAsync(_lightBulbCommandsApiUrl,
                new LightBulbCommand()
                {
                    LightBulbId = Guid.Parse("0365f802-bb3a-487a-997b-0d34c270a385"),
                    Color = 6556210,
